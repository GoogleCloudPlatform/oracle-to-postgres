...
then
    docker run -v ${PWD}/data_validation/.config/:/root/.config --rm data-validation \
...
then
    export TABLES_LIST=$(docker run -v ${PWD}/data_validation/.config/:/root/.config --rm data-validation find-tables --source-conn oracle --target-conn postgres)
    docker run -v ${PWD}/data_validation/.config/:/root/.config --rm data-validation \
        run --source-conn oracle --target-conn postgres --tables-list "${TABLES_LIST}" --type Column
elif [ "$1" == "destroy" ]
then
    docker rmi -f data-validation
    rm -rf data_validation/.config/
else
    docker run -v ${PWD}/data_validation/.config/:/root/.config --rm data-validation --help
fi
